...
export function validatePortfolioAssets(assets) {
  ...
  return assets.map((asset, index) => {
    ...
    return {
      ...
      position: {
        quantity: parseFiniteNumber(position.quantity ?? 0, `assets[${index}].position.quantity`, { min: 0 }),
        averageCost: parseFiniteNumber(position.averageCost ?? asset.price ?? 0, `assets[${index}].position.averageCost`, { min: 0 }),
