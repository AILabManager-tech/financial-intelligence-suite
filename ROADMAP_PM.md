...
| P4.3 MWR / IRR ✅ | Newton-Raphson sur flux nets, annualisé. **Livré 2026-06-01** : `computeMoneyWeightedReturn` pur (IRR par Newton-Raphson + repli bisection sur les flux investisseur datés −V_début/−apport/+retrait/+V_fin ; MWR de période toujours donné, IRR annualisé gated ≥ 1 an ; masqué si pas de convergence), `PortfolioMwrPanel` (dashboard, `performance`). Effet timing client (MWR) vs effet gérant (TWR). Dogfood : MWR +1,38 % = TWR (sans flux, cohérent). | S | P4.2 |
| P4.4 Volatilité + drawdown + duration ✅ | σ annualisée sur l'espacement réel, fenêtres 30j/90j/1a/inception, max DD + duration de récupération. **Livré 2026-06-01** : `computePortfolioRisk` pur (σ d'échantillon des rendements flux-neutralisés, annualisée ×√(365/jours moyens) pour gérer l'espacement réel — corrigé le 2026-08-08, la version 252 sous-estimait de ~15 % ; repli max pic→creux + dates + durée de récupération + repli courant/statut), `PortfolioRiskPanel` (dashboard, `performance`). Fenêtre = inception (série jeune) ; fenêtres 30j/90j/1a = refinement quand la série s'allonge. Dogfood : σ 9,42 %, DD max −1,69 %, récupéré en 9 j. | M | P4.2 |
| P4.5 Sharpe + Sortino + Calmar ✅ | Taux sans risque configurable. **Livré 2026-06-01** : `computePortfolioRatios` pur (Sharpe = excès/σ ×√périodes, Sortino = excès/déviation à la baisse, Calmar = rendt annualisé/|repli max| gated ≥ 1 an), réutilise `computeSubPeriodReturns` + `computePortfolioRisk` ; taux sans risque = hypothèse étiquetée (défaut 0 %). `PortfolioRatiosPanel` (dashboard, `performance`). Dogfood : Sharpe 1,79 / Sortino 2,80 (rf=0). | S | P4.4 |
| P4.6 Benchmark (S&P 500, TSX, MSCI ACWI, custom) ✅ | Choix par portefeuille, excess return. **Livré 2026-06-01** : `computeBenchmarkComparison` pur (TWR portefeuille vs rendement de prix du benchmark sur la même fenêtre [from,to], excès ; clôture on-or-before, masqué si série benchmark ne couvre pas), `BenchmarkPanel` (dashboard `performance`, sélecteur SPY/QQQ/DIA, fetch `/api/history`). | M | P4.2 |
| P4.7 Beta + corrélation inter-positions ✅ | Régression OLS vs benchmark, matrice Pearson. **Livré 2026-06-01** : `computeBenchmarkStats`/`pairBenchmarkReturns` purs (apparie les rendements de sous-période portefeuille au prix benchmark sur le même intervalle, régression OLS → beta/corrélation/R², null si variance benchmark nulle), `BetaCorrelationPanel` (dashboard `performance`, sélecteur SPY/QQQ/DIA). Matrice Pearson inter-positions déjà livrée en P5.10. | M | P4.4, P4.6 |
| P4.8 Ratios étendus vs benchmark ✅ | Jensen's alpha, tracking error, information ratio, R², up/down capture, Treynor. **Livré 2026-06-01** : `BenchmarkRatiosPanel` (dashboard `performance`) consomme `computeBenchmarkStats` (alpha de Jensen annualisé, TE annualisée, IR, Treynor, up/down capture ; null si dénominateur nul) ; taux sans risque = hypothèse étiquetée. Sélecteur SPY/QQQ/DIA. | M | P4.7 |
| P4.9 Attribution Brinson ⏸️ BLOQUÉ-DONNÉES | Sélection + allocation + interaction, par secteur et devise. **NON livré 2026-06-01 (décision factualité)** : Brinson exige les **poids ET rendements sectoriels du benchmark** ; aucune source factuelle disponible (Finnhub free ne donne pas la composition d'un indice). Fabriquer ces poids violerait la factualité stricte (règle #1). À rouvrir si une source réelle de composition d'indice est branchée (ETF holdings premium, ou saisie manuelle d'une politique d'allocation cible). | L | P4.6, P3.4 |
| P4.10 Distribution des returns | Best/worst périodes, % mois positifs, skewness/kurtosis, heatmap monthly. | M | P4.1 |
| P4.11 VaR / CVaR ✅ | VaR paramétrique + historique, 95/99 %, CVaR. **Livré 2026-06-01** : `computeValueAtRisk` pur (VaR paramétrique μ−z·σ toujours, VaR + CVaR historiques dès 10 obs, sur les rendements flux-neutralisés), `ValueAtRiskPanel` (dashboard `performance`, table 95/99 %). Horizons « 1j/10j » écartés (périodes irrégulières) au profit d'une base « par période » honnête. | M | P4.4 |
...
| P5.3 Rééquilibrage avec coûts ✅ | Suggestion d'ordres au target. **Livré 2026-06-01** : `computeRebalance` pur (dérive vs `targetWeight`, ordre achat/vente en $ pour rejoindre la cible, seuil de dérive = proxy coûts qui supprime les micro-ajustements, cibles prises telles quelles + signale le cash implicite si Σ<100 %), `RebalancePanel` (dashboard `portfolio`, table dérive/action). « Hypothèse, pas un conseil ». | M | P5.2, P3.3 |
...
| P5.6 Macro (taux, inflation, courbe) ✅ | Fed via FRED (gratuit). **Livré 2026-06-01** : `fetchMacroIndicators` (FRED `/series/observations`, dernier point de FEDFUNDS/DGS2/DGS10/T10Y2Y, `Promise.allSettled` dégradé), `api/macro` + middleware (cache 6h, `FRED_API_KEY` optionnelle documentée), `MacroPanel` (dashboard `monitoring`, état honnête sans clé). Inflation YoY + BoC + tags d'exposition = extension future. | M | — |
| P5.7 ESG / insider / short interest (insider + sentiment ✅) | Fallback Yahoo `esgScores` ; Finnhub insider + short interest. **Insider transactions livré 2026-05-31** (`InsiderTransactionsPanel`). **Insider sentiment MSPR livré 2026-06-01** : `fetchInsiderSentiment` Finnhub `/stock/insider-sentiment` (free) + `InsiderSentimentPanel` (fiche actif `sentiment`), MSPR mensuel + moyenne 12 mois + label accumulation/distribution, pattern 7-fichiers. ⏸️ **Reste bloqué-données** : short interest (Finnhub premium, pas en free tier) et ESG (Yahoo `esgScores` = scraping fragile non branché) — non fabriqués. | M | — |
...
> **P3.4 Import du journal de transactions ✅ (2026-07-17)** — hors table, ajouté après coup : répond à « pourquoi utiliser cette appli alors que iA/Croesus existe et est sécurisé ? ». Réponse : FIS ne doit pas être une app de portefeuille (il perd contre le système d'enregistrement), mais une **couche par-dessus un export**. `src/services/transactionCsvImporter.js` : import bilingue FR/EN d'un relevé de courtier → journal de transactions, branché dans `/transactions` avec aperçu avant écriture. Débloque le moteur de lots, les gains réalisés et le rapport fiscal **dès le jour 1** au lieu de la saisie manuelle. **Suite critique livrée (2026-07-17)** : `src/utils/reconstructSnapshots.js` pur reconstruit la série de valeurs (quantités du journal × clôtures réelles via `closeOnOrBefore`, extrait vers `src/utils/priceSeries.js`) ⇒ le brief « depuis la dernière rencontre » et le TWR s'allument au démarrage à froid, sans attendre l'accumulation. Un jour sans clôture pour un titre détenu est omis, jamais interpolé ; la période reconstruite est étiquetée. Câblée dans `MeetingBriefView` (les relevés réels priment dès qu'il y en a ≥ 2). Détails dans `REPRISE_CHECKPOINT.md`.
...
| P6.6 Sujets probables de rencontre ✅ | **Première intégration LLM du projet.** Parmi l'actualité des titres détenus, sélectionne les sujets que le client va soulever — aucune fonction pure ne le fait (jugement sur texte non structuré). **Livré 2026-07-16** : `server/meetingTopics.js` (`@anthropic-ai/sdk`, modèle `claude-opus-4-8`, thinking adaptatif, structured outputs), endpoint batch `/api/meeting-topics?symbols=`, injecté dans `buildMeetingBrief`. **Factualité** : le modèle n'émet aucun fait — il SÉLECTIONNE parmi des articles fournis et CITE des références qu'on lui a attribuées ; toute citation inconnue est jetée et un sujet non sourcé est supprimé (`dropped`). Clé optionnelle : absente ⇒ section absente, jamais fabriquée. | M | P6.5 |
| P6.7 Agent de préparation (additif, hors app) ✅ | **Vrai agent** (≠ P6.6 qui est un appel unique) : outils + boucle pilotée par le modèle via `client.beta.messages.toolRunner`. Le modèle choisit quoi creuser — avec 52 positions il ne peut pas tout regarder. **Livré 2026-07-16** : `server/briefAgent.js` (3 outils : `list_positions`, `get_company_news`, `get_earnings_calendar`), CLI `npm run agent -- --portfolio <export.json>` consommant le JSON que l'app exporte déjà. **Plug-and-play** : importé par aucun module de l'app ; sans clé, inerte. **Garanties** : garde-fou de périmètre (refuse un titre non détenu) + trace complète des appels d'outils. ⚠️ Trace = garantie **plus faible** que la citation vérifiée de P6.6 — c'est le prix de l'agentivité, d'où : l'agent commente, il ne calcule pas. | M | P6.6 |
