...
| P4.4 Volatilité + drawdown + duration ✅ | σ × √252, fenêtres 30j/90j/1a/inception, max DD + duration de récupération. **Livré 2026-06-01** : `computePortfolioRisk` pur (σ d'échantillon des rendements flux-neutralisés, annualisée ×√(252/jours moyens) pour gérer l'espacement réel ; repli max pic→creux + dates + durée de récupération + repli courant/statut), `PortfolioRiskPanel` (dashboard, `performance`). Fenêtre = inception (série jeune) ; fenêtres 30j/90j/1a = refinement quand la série s'allonge. Dogfood : σ 9,42 %, DD max −1,69 %, récupéré en 9 j. | M | P4.2 |
