...
- **`main` = P5.7 transactions d'initiés actif (ce bloc, NON poussé).** `origin/main` = `17efb32` (P5.9 drawdown + bloc précédent poussés). ⚠️ **P5.10 corrélation `e0c06e8` + P5.7 transactions d'initiés = 2 commits devant origin, NON poussés.** Push après simple avertissement, jamais `--force`. Branche `main`.
- **751 tests verts** (P5.10 : →714 ; P5.7 : 714→751, +37 : 9 server + 7 service + 14 formatters + 7 panel), lint + build verts. Node v24 via nvm : `export PATH="$HOME/.nvm/versions/node/v24.14.1/bin:$PATH"`.
- **Avancement** : 🏁 Phase 0 (noyau personnalisable) · Phase 1 P1.1 (agencement déterministe ; **P1.2 IA gelé**) · 🏁 Phase 2 (simulateur de démo) · 🏁 Phase 3 (socle données PM) · **Phase 4 : features hors-snapshots livrées (P4.1 ✅ + P4.10 ✅ + P4.12 ✅)** · **Phase 5 en cours : P5.1 journal ✅ + P5.7 initiés (partiel) ✅ + P5.8 concentration ✅ + P5.9 drawdown ✅ + P5.10 corrélation ✅**.
- **Cap autonome (directive user 2026-05-30)** : en l'absence de l'utilisateur, **prioriser la modularité/personnalisation** — enrichir le catalogue de features attachables (enregistrées au registre, activables/positionnables indépendamment), pas l'empilage analytique. Cf. mémoire `feedback_prioritize_modularity.md`. P5.7 est choisi car **purement additif, pattern 7-fichiers canonique, capability factuelle réellement neuve (Finnhub déjà câblé = zéro source neuve)** — préféré à un 5e panel dérivé de `/api/history` (qui chevaucherait la distribution P4.10) et à P5.2/P5.4 (qui *mutent* le flux existant).
- **➡️ PROCHAIN BLOC (choisir SEUL)** : (A) **accrual de snapshots SQLite** (brique socle, débloque P4.2 TWR / P4.3 MWR / P4.4 vol / P4.5 Sharpe — table `snapshots` présente, non peuplée ; factualité : ne jamais fabriquer la série de valeur) — réservé à quand l'utilisateur veut le factsheet portefeuille. (B) feature de catalogue alignée modularité : compléter P5.7 (short-interest / insider-sentiment Finnhub, ESG via fallback Yahoo), P5.6 macro FRED (M, source gratuite neuve), ou autre panel additif. P5.2 contraintes/compliance et P5.4 watchlists *touchent le flux existant* (moins propres en modularité pure). **Défaut si hésitation : (B).** Lire `ROADMAP_PM.md` § Prochain bloc.
