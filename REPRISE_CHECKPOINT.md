...
- **`main` = `b41f029`, P3.2c livré et poussé sur `origin/main`** (0 commit local d'écart). Branche `main`.
- **561 tests verts**, lint + build verts. Node v24 via nvm : `export PATH="$HOME/.nvm/versions/node/v24.14.1/bin:$PATH"`.
- **Avancement** : 🏁 Phase 0 (noyau personnalisable) · Phase 1 P1.1 (agencement déterministe ; **P1.2 IA gelé**) · 🏁 Phase 2 (simulateur de démo) · **Phase 3 en cours** : P3.1 migrations ✅, P3.2 multi-portefeuilles ✅ (client-first, vérifié live), P3.3a moteur de lots pur ✅, P3.3b stockage transactions + journal UI ✅ (route `/transactions`, vérifié live FIFO/LIFO), P3.2c parité dev SQLite multi-portefeuille ✅ (dev-only, vérifié curl).
- **➡️ PROCHAIN BLOC : P3.4 — multi-devises + FX.** Provider de taux externe (ECB / exchangerate.host), nouvel endpoint dev (`vite.config.js`) + prod (`api/fx.js`), conversion de la valeur de marché et du P&L réalisé/non-réalisé vers la devise base du mandat (`baseCurrency`, déjà stockée client + dev SQLite). Première vraie nouveauté fonctionnelle depuis P3.2.
- **Reste Phase 3 après P3.4** : brancher `transactionStore` côté serveur (repo + API dev) pour la persistance dev des transactions (parité optionnelle), puis clôture de Phase 3.
