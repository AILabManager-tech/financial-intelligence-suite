...
- **914 tests verts**. 🏁 Phase 4 + Phase 5 (factuel) poussées. **PHASE 6 : P6.2 snapshots fiscaux ✅ (poussé) + P6.1 reporting de mandat / PDF ✅ (commit en cours, NON poussé).** ➡️ Restent en Phase 6 : P6.3 commentaire PM daté (S, dépend P6.1 → maintenant débloqué), P6.4 portail client read-only (dépend P6.1 + P7.1 auth → bloqué tant que pas d'auth Phase 7). Bloqués-données persistants : P5.5 SEDAR+ & dividendes canadiens (majoration/crédit) ; P5.7 short-interest premium + ESG scraping ; P4.9 Brinson (composition sectorielle benchmark). Action différée (fin de projet) : **rotation des 3 clés API**.
