...
- **`main` = P4.4 risque PF actif (ce bloc, NON poussé).** `origin/main` = `17efb32`. ⚠️ **6 commits devant origin, NON poussés** : P5.10 `e0c06e8`, P5.7 `182e294`, générateur `acf317e`, accrual `9188f93`, P4.2 TWR `bfb0ba7`, P4.4 risque PF (feat). Push après simple avertissement, jamais `--force`. Branche `main`.
- **773 tests verts** (P4.2 : →766 ; P4.4 : 766→773, +7 : 4 util portfolioRisk + 3 panel), lint + build verts. Node v24 via nvm : `export PATH="$HOME/.nvm/versions/node/v24.14.1/bin:$PATH"`.
- **Avancement** : 🏁 Phase 0 (noyau personnalisable) · Phase 1 P1.1 (agencement déterministe ; **P1.2 IA gelé**) · 🏁 Phase 2 (simulateur de démo) · 🏁 Phase 3 (socle données PM) · **Phase 4 : P4.1 ✅ + P4.10 ✅ + P4.12 ✅ + 🔓 accrual snapshots ✅ + P4.2 TWR ✅ + P4.4 vol/drawdown PF ✅ → restent P4.3 MWR/IRR, P4.5 Sharpe (débloqués), P4.6 benchmark, P4.7-4.9, P4.11** · **Phase 5 : P5.1 ✅ + P5.7 initiés (partiel) ✅ + P5.8 ✅ + P5.9 ✅ + P5.10 ✅**.
...
- **➡️ PROCHAIN BLOC (choisir SEUL)** : suite Phase 4 sur la série de snapshots — **P4.5 Sharpe/Sortino/Calmar** (ratios sur les rendements flux-neutralisés + taux sans risque ; Calmar = rendement annualisé / |max DD| déjà calculé par P4.4) ou **P4.3 MWR/IRR** (Newton-Raphson sur les flux nets datés + valeur finale, effet timing). **Défaut : P4.5 Sharpe** (réutilise directement `computeSubPeriodReturns` + le DD de P4.4, le plus additif ; taux sans risque = hypothèse étiquetée, pas une donnée live). Alternative (B) : catalogue additif (P5.7 short-interest, P5.6 macro FRED).
