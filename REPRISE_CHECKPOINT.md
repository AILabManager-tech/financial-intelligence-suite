...
- **`main` = P4.12 (`cacc6db`) + P5.1 (ce bloc, journal d'investissement).** ⚠️ **P4.12 ET P5.1 NON poussés** — `main` est 2 commits devant `origin/main` (`d34cd49` = P4.10). Push après simple avertissement, jamais `--force`. Branche `main`.
- **677 tests verts** (P4.12 : →647 ; P5.1 : 647→677, +30 : 14 store + 9 formatters + 7 panel), lint + build verts. Node v24 via nvm : `export PATH="$HOME/.nvm/versions/node/v24.14.1/bin:$PATH"`.
- **Avancement** : 🏁 Phase 0 (noyau personnalisable) · Phase 1 P1.1 (agencement déterministe ; **P1.2 IA gelé**) · 🏁 Phase 2 (simulateur de démo) · 🏁 Phase 3 (socle données PM) · **Phase 4 : features hors-snapshots livrées (P4.1 ✅ + P4.10 ✅ + P4.12 ✅)** · **Phase 5 amorcée : P5.1 journal d'investissement ✅**.
- **Cap autonome (directive user 2026-05-30)** : en l'absence de l'utilisateur, **prioriser la modularité/personnalisation** — enrichir le catalogue de features attachables (enregistrées au registre, activables/positionnables indépendamment), pas l'empilage analytique. Cf. mémoire `feedback_prioritize_modularity.md`. P5.1 est choisi car **purement additif** (P5.4 watchlists aurait muté la watchlist existante → viole « zéro modif des autres features »).
- **➡️ PROCHAIN BLOC (choisir SEUL)** : (A) **accrual de snapshots SQLite** (brique socle, débloque P4.2 TWR / P4.3 MWR / P4.4 vol / P4.5 Sharpe — table `snapshots` présente, non peuplée ; factualité : ne jamais fabriquer la série de valeur) — réservé à quand l'utilisateur veut le factsheet portefeuille. (B) feature de catalogue alignée modularité : P5.2 contraintes/compliance (M), P5.4 watchlists thématiques (S mais mute l'existant), ou un panel dashboard dérivé des positions/transactions. **Défaut si hésitation : (B).** Lire `ROADMAP_PM.md` § Prochain bloc.
- **P5.1 en bref** : `src/services/investmentJournalStore.js` (localStorage `fis:investment-journal:v1`, map par symbole, mutateurs purs upsert/remove, note vidée = supprimée) + `src/utils/investmentJournalFormatters.js` (conviction `n/5` + label, `reviewStatus` overdue/soon/scheduled avec `today` injecté = pur) + `src/components/InvestmentJournalPanel.jsx` (formulaire fiche actif : thèse, conviction 1-5, prix cible, stop, date de revue ; persistance auto ; reload sur changement de symbole via garde de rendu, **pas** d'effet → conforme `react-hooks/set-state-in-effect`). Registre catégorie **`decisions`** (surface actif, order 115), `layoutEngine` priorité 25. **Zéro fichier serveur** (comme la watchlist). Factualité : champs absents masqués (—), cible/stop = objectifs du user étiquetés « pas un conseil ».
