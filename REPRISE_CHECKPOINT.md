...
**Recommandation par défaut sur « on continue »** : **P0.4 — onglet Paramètres `/settings`**. L'UI d'édition du layout : liste des features par catégorie (`groupFeaturesByCategory`) + toggle on/off + drag-and-drop de repositionnement + aperçu live + bouton reset. Branche les mutateurs déjà livrés (`setFeatureVisibility`, `moveFeature`, `setFeatureColumns`, `saveLayout`, `resetLayout`). ⚠️ Prérequis : `useLayout` (P0.3) lit le store une seule fois au montage — P0.4 doit le rendre réactif (contexte React ou abonnement) pour que toggles/réordo re-rendent `IntelligenceCard` et le dashboard. C'est le cœur du travail (persistance + rendu existent déjà). Le rendu du colonnage 1/2 (persisté, pas encore gridé dans `LayoutSurface`) peut être branché ici. Effort L, dépend de P0.1+P0.2+P0.3 (livrés).
...
- **P0.4** onglet Paramètres `/settings` : toggles on/off + drag-and-drop de repositionnement + aperçu live + reset — *recommandation par défaut, ci-dessus*.
