...
- **907 tests verts**. 🏁 Phase 4 + Phase 5 (factuel) poussées. **PHASE 6 DÉMARRÉE : P6.2 snapshots fiscaux annuels livré (commit en cours, NON poussé).** ➡️ Restent en Phase 6 : P6.1 reporting PDF (L, lib `@react-pdf/renderer`, dépend partiellement de P4.9 Brinson bloqué-données → PDF possible sans l'attribution), P6.3 commentaire PM (S, dépend P6.1), P6.4 portail client read-only (dépend P6.1 + P7.1 auth). Bloqués-données persistants : P5.5 SEDAR+ & dividendes canadiens (majoration/crédit) ; P5.7 short-interest premium + ESG scraping. Action différée (fin de projet) : **rotation des 3 clés API**.
