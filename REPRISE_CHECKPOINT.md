...
- **897 tests verts**. 🏁 Phase 4 poussée. **Phase 5 livré : P5.2 + P5.3 + P5.4 watchlists + P5.5 couverture canadienne (factuel) + P5.6 + P5.7b — tous poussés. PLUS débloquage retenue US sur dividendes (ex-bloqué-données P5.5).** ➡️ **Phase 5 close côté factuel.** Restent **bloqués-données** (non fabriqués, documentés) : P5.5 SEDAR+ & traitement fiscal des dividendes canadiens (majoration/crédit) ; P5.7 short-interest (Finnhub premium) + ESG (Yahoo scraping). Action différée (fin de projet, décision user) : **rotation des 3 clés API** (repo a été public). Prochains candidats : autre feature de catalogue dérivable des données existantes, ou Phase 6.
