...
- **`main` = P4.5 ratios actif (ce bloc, NON poussé).** `origin/main` = `17efb32`. ⚠️ **7 commits devant origin, NON poussés** : P5.10 `e0c06e8`, P5.7 `182e294`, générateur `acf317e`, accrual `9188f93`, P4.2 TWR `bfb0ba7`, P4.4 risque PF `5a13a9f`, P4.5 ratios (feat). Push après simple avertissement, jamais `--force`. Branche `main`.
- **780 tests verts** (P4.4 : →773 ; P4.5 : 773→780, +7 : 5 util portfolioRatios + 2 panel), lint + build verts. Node v24 via nvm : `export PATH="$HOME/.nvm/versions/node/v24.14.1/bin:$PATH"`.
- **Avancement** : 🏁 Phase 0 (noyau personnalisable) · Phase 1 P1.1 (agencement déterministe ; **P1.2 IA gelé**) · 🏁 Phase 2 (simulateur de démo) · 🏁 Phase 3 (socle données PM) · **Phase 4 : P4.1 ✅ + P4.10 ✅ + P4.12 ✅ + 🔓 accrual snapshots ✅ + P4.2 TWR ✅ + P4.4 vol/drawdown PF ✅ + P4.5 Sharpe/Sortino/Calmar ✅ → restent P4.3 MWR/IRR (débloqué), P4.6 benchmark, P4.7-4.9, P4.11** · **Phase 5 : P5.1 ✅ + P5.7 initiés (partiel) ✅ + P5.8 ✅ + P5.9 ✅ + P5.10 ✅**.
...
- **➡️ PROCHAIN BLOC (choisir SEUL)** : **P4.3 MWR/IRR** (dernier ratio perf manquant : Newton-Raphson sur les flux nets datés `computeFlowsByDay` + valeur initiale/finale des snapshots → IRR annualisé ; effet timing client vs effet PM ; masquer si pas de convergence ou < 2 flux). Puis **P4.6 benchmark** (overlay S&P 500/TSX via `/api/history` d'un symbole de référence, excess return vs TWR) qui ouvre P4.7-4.9. **Défaut : P4.3 MWR/IRR** (clôt le quatuor de rendement perf ; réutilise `computeFlowsByDay` déjà exporté). Alternative (B) : catalogue additif (P5.7 short-interest, P5.6 macro FRED). Note : P4.2/4.4/4.5 partagent `computeSubPeriodReturns` — primitif factuel unique, réutiliser pour tout calcul de rendement de marché.
