...
- **898 tests verts**. 🏁 Phase 4 poussée. **Phase 5 livré : P5.1→P5.10 côté factuel — P5.2 + P5.3 + P5.4 watchlists + P5.5 (canadien factuel + retenue US) + P5.6 (macro, ÉTENDU : inflation US + taux BdC) + P5.7b + P5.8/9/10 — poussés sauf le dernier bloc macro (commit en cours).** ➡️ **Phase 5 close côté factuel.** Restent **bloqués-données** (non fabriqués, documentés) : P5.5 SEDAR+ & traitement fiscal des dividendes canadiens (majoration/crédit) ; P5.7 short-interest (Finnhub premium) + ESG (Yahoo scraping). Action différée (fin de projet, décision user) : **rotation des 3 clés API** (repo a été public). Prochains candidats : Phase 6 (exploitation client — PDF + fiscal), ou autre feature de catalogue dérivable des données existantes.
