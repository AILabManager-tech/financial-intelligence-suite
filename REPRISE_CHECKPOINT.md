...
- **`main` = accrual journalier de snapshots actif (ce bloc, NON poussé).** `origin/main` = `17efb32`. ⚠️ **4 commits devant origin, NON poussés** : P5.10 corrélation `e0c06e8`, P5.7 initiés `182e294`, générateur de scaffolding (chore), accrual de snapshots (feat). Push après simple avertissement, jamais `--force`. Branche `main`.
- **753 tests verts** (P5.7 : →751 ; accrual : 751→753, +2 : 1 migrate 005 + 1 repo multi-jours, 1 test repo réécrit pour le nouveau contrat journalier), lint + build verts. Node v24 via nvm : `export PATH="$HOME/.nvm/versions/node/v24.14.1/bin:$PATH"`.
- **Avancement** : 🏁 Phase 0 (noyau personnalisable) · Phase 1 P1.1 (agencement déterministe ; **P1.2 IA gelé**) · 🏁 Phase 2 (simulateur de démo) · 🏁 Phase 3 (socle données PM) · **Phase 4 : features hors-snapshots livrées (P4.1 ✅ + P4.10 ✅ + P4.12 ✅) + 🔓 ACCRUAL DE SNAPSHOTS POSÉ → P4.2/4.3/4.4/4.5 désormais débloqués** · **Phase 5 en cours : P5.1 journal ✅ + P5.7 initiés (partiel) ✅ + P5.8 concentration ✅ + P5.9 drawdown ✅ + P5.10 corrélation ✅**.
- **Outillage neuf (ce bloc)** : `scripts/new-feature.sh` — générateur de scaffolding déterministe (7-fichiers + snippets de câblage à coller). Tue le fastidieux de la plomberie SANS risque factuel ; le jugement (source, anti-chevauchement, factualité) reste manuel. **Décision de fond actée** : ne PAS mandater un agent autonome (ex. Hermès `--yolo`) pour pondre les blocs — le goulot est le jugement + relecture, pas la frappe ; dérive silencieuse sur règles strictes. Cf. `~/.claude/CLAUDE.md` § Hermès.
- **➡️ PROCHAIN BLOC (choisir SEUL)** : maintenant que la série de valeur journalière s'accumule factuellement, la voie naturelle est **P4.2 TWR** (rendement pondéré-temps GIPS sur la série de snapshots) puis P4.3 MWR/IRR, P4.4 vol/drawdown PORTEFEUILLE, P4.5 Sharpe — chacun un panel dashboard dérivé de `listSnapshots`. **Pré-requis factuel rappelé** : ne calculer que sur les jours réellement accumulés (pas de backfill) ; masquer les fenêtres (1A, 3A…) tant que la série ne remonte pas si loin. Alternative (B) : features de catalogue additives (compléter P5.7 short-interest, P5.6 macro FRED). **Défaut : P4.2 TWR** (le socle vient d'être posé exprès pour ça).
