...
- **`main` = P4.2 TWR actif (ce bloc, NON poussé).** `origin/main` = `17efb32`. ⚠️ **5 commits devant origin, NON poussés** : P5.10 `e0c06e8`, P5.7 `182e294`, générateur `acf317e`, accrual `9188f93`, P4.2 TWR (feat). Push après simple avertissement, jamais `--force`. Branche `main`.
- **766 tests verts** (accrual : →753 ; P4.2 : 753→766, +13 : 9 util TWR + 4 panel), lint + build verts. Node v24 via nvm : `export PATH="$HOME/.nvm/versions/node/v24.14.1/bin:$PATH"`.
- **Avancement** : 🏁 Phase 0 (noyau personnalisable) · Phase 1 P1.1 (agencement déterministe ; **P1.2 IA gelé**) · 🏁 Phase 2 (simulateur de démo) · 🏁 Phase 3 (socle données PM) · **Phase 4 : P4.1 ✅ + P4.10 ✅ + P4.12 ✅ + 🔓 accrual snapshots ✅ + P4.2 TWR ✅ → restent P4.3 MWR/IRR, P4.4 vol/drawdown PF, P4.5 Sharpe (tous débloqués), P4.6 benchmark, P4.7-4.9, P4.11** · **Phase 5 : P5.1 ✅ + P5.7 initiés (partiel) ✅ + P5.8 ✅ + P5.9 ✅ + P5.10 ✅**.
...
- **➡️ PROCHAIN BLOC (choisir SEUL)** : suite naturelle Phase 4 sur la série de snapshots — **P4.3 MWR/IRR** (Newton-Raphson sur flux nets, effet timing) ou **P4.4 vol/drawdown PORTEFEUILLE** (σ×√252 + max DD sur la série de valeur) ou **P4.5 Sharpe**. Tous dérivent de `listSnapshots` (+ flux pour MWR), même pattern que P4.2 TwrPanel. **Pré-requis factuel** : ne calculer que sur les jours réellement accumulés, masquer les fenêtres longues tant que la série ne remonte pas si loin. **Défaut : P4.4 vol/drawdown PF** (pur, ne dépend pas des flux, dérivé direct de la série). Alternative (B) : catalogue additif (P5.7 short-interest, P5.6 macro FRED).
