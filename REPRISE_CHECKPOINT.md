...
- **919 tests verts**. 🏁 Phase 4 + Phase 5 (factuel) poussées. **PHASE 6 : P6.2 snapshots fiscaux ✅ + P6.1 reporting/PDF ✅ (poussés) + P6.3 commentaire PM daté ✅ (commit en cours, NON poussé).** ➡️ Reste en Phase 6 : **P6.4 portail client read-only — BLOQUÉ tant que l'auth (P7.1) n'existe pas** (URL signée par mandat). Phase 6 livrable solo = essentiellement complète (P6.1/6.2/6.3) ; P6.4 attend Phase 7. Bloqués-données persistants : P5.5 SEDAR+ & dividendes canadiens (majoration/crédit) ; P5.7 short-interest premium + ESG scraping ; P4.9 Brinson (composition sectorielle benchmark). Action différée (fin de projet) : **rotation des 3 clés API**.
