...
- **853 tests verts** (P5.7b : →844 ; P5.6 : 844→853, +9 : 4 server macro + 3 service + 2 panel). 🏁 Phase 4 poussée (`6d791b7`). **Phase 5 en cours : P5.2 `8eb40d1` ✅ + P5.3 `05339ca` ✅ + P5.7b `d8df925` ✅ + P5.6 macro FRED ✅** (commit en cours). Reste Phase 5 : P5.4 watchlists thématiques, P5.5 Canada (L). ⏸️ P5.7 short-interest (premium) + ESG (Yahoo scraping) = bloqués-données. **🏁 PHASE 4 COMPLÈTE** (sauf P4.9 Brinson). Blocs session autonome 2026-06-01 : P4.3 `d97d642`, P4.6 `8be14bc`, P4.7 `df81bcd`, P4.8 `9b3a7d1`, P4.11 (commit en cours). ⚠️ **P4.9 Brinson DÉLIBÉRÉMENT NON LIVRÉ** : exige poids+rendements sectoriels du benchmark, aucune source factuelle (Finnhub free ne donne pas la composition d'un indice) → fabriquer violerait la factualité stricte. Documenté bloqué-sur-données, jamais de mock. ➡️ **Suite : PHASE 5** (P5.2 compliance, P5.3 rééquilibrage, P5.4 watchlists, P5.5 Canada, P5.6 macro FRED, P5.7 short-interest/ESG — compléter le sous-bloc initiés). lint + build verts. Node v24 via nvm : `export PATH="$HOME/.nvm/versions/node/v24.14.1/bin:$PATH"`.
