...
- **`main` = P5.8 (`4deead2`) poussé + fix câblage P4.12 (ce bloc, NON poussé).** `origin/main` = `4deead2` (P5.1 + P5.8 + fix `.env.example` poussés). ⚠️ **Le fix P4.12 = commit de ce bloc, NON poussé** (1 commit devant origin). Push après simple avertissement, jamais `--force`. Branche `main`.
- **692 tests verts** (P5.8 : →689 ; fix P4.12 : 689→692, +3 : test de complétude `dashboardPanelProps`), lint + build verts. Node v24 via nvm : `export PATH="$HOME/.nvm/versions/node/v24.14.1/bin:$PATH"`.
- **Avancement** : 🏁 Phase 0 (noyau personnalisable) · Phase 1 P1.1 (agencement déterministe ; **P1.2 IA gelé**) · 🏁 Phase 2 (simulateur de démo) · 🏁 Phase 3 (socle données PM) · **Phase 4 : features hors-snapshots livrées (P4.1 ✅ + P4.10 ✅ + P4.12 ✅)** · **Phase 5 en cours : P5.1 journal d'investissement ✅ + P5.8 concentration & diversification ✅**.
- **Cap autonome (directive user 2026-05-30)** : en l'absence de l'utilisateur, **prioriser la modularité/personnalisation** — enrichir le catalogue de features attachables (enregistrées au registre, activables/positionnables indépendamment), pas l'empilage analytique. Cf. mémoire `feedback_prioritize_modularity.md`. P5.8 est choisi car **purement additif et dérivé des positions existantes** (zéro source neuve), comme P4.12.
