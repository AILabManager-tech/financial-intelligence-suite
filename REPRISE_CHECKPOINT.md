...
- **`main` = P4.1 livré (commit de ce bloc), Phase 4 entamée.** ⚠️ **Non poussé** — push après simple avertissement, sur demande. Avant ce bloc `origin/main` = `b146c8d`. Branche `main`.
- **611 tests verts** (593 → +18 : 9 calculator + 5 formatters + 4 panel), lint + build verts. Node v24 via nvm : `export PATH="$HOME/.nvm/versions/node/v24.14.1/bin:$PATH"`.
- **Avancement** : 🏁 Phase 0 (noyau personnalisable) · Phase 1 P1.1 (agencement déterministe ; **P1.2 IA gelé**) · 🏁 Phase 2 (simulateur de démo) · 🏁 Phase 3 (socle données PM) · **Phase 4 entamée : P4.1 rendements standards ✅**.
- **➡️ PROCHAIN BLOC (candidats, choisir SEUL)** : **P4.10 distribution des returns** (best/worst, % mois positifs, skewness/kurtosis, heatmap monthly — extension factuelle de `ReturnsMatrixPanel` sur la même série `/api/history`) **ou P4.12 stats opérationnelles** (turnover, holding period, hit ratio, win/loss, yield-on-cost — branché sur le moteur de lots P3.3a). Les deux dérivables **sans** snapshots. ⚠️ **P4.2+ (TWR/MWR/vol/Sharpe portefeuille) bloqués sur l'accrual de snapshots SQLite** (table présente, non peuplée) — factualité : ne pas fabriquer de série de valeur portefeuille avant. Lire `ROADMAP_PM.md` § Prochain bloc.
- **P4.1 en bref** : `src/utils/returnsCalculator.js` (pur) + `returnsFormatters.js` + `src/components/ReturnsMatrixPanel.jsx` (surface actif, registre catégorie `performance`, `layoutEngine` priorité 15). Rendement cumulé + CAGR + matrice par période (1M→origine, périodes hors-données **masquées**) + rendements mensuels, depuis `/api/history` (days=1825). Rendements de prix hors dividendes (mention affichée). Réutilise l'endpoint history existant (zéro nouveau fichier serveur, comme SimulationPanel).
