...
- **`main` = P4.1 + P4.10 livrés, Phase 4 en cours.** ⚠️ **P4.1 poussé (`d17fe32`) ; P4.10 = commit de ce bloc, NON poussé** (push après simple avertissement). Branche `main`.
- **624 tests verts** (P4.1 : 593→611 ; P4.10 : 611→624, +13 : 7 distribution + 2 formatters + 4 panel), lint + build verts. Node v24 via nvm : `export PATH="$HOME/.nvm/versions/node/v24.14.1/bin:$PATH"`.
- **Avancement** : 🏁 Phase 0 (noyau personnalisable) · Phase 1 P1.1 (agencement déterministe ; **P1.2 IA gelé**) · 🏁 Phase 2 (simulateur de démo) · 🏁 Phase 3 (socle données PM) · **Phase 4 : P4.1 rendements standards ✅ + P4.10 distribution des rendements ✅**.
- **Cap autonome (directive user 2026-05-30)** : en l'absence de l'utilisateur, **prioriser la modularité/personnalisation** — enrichir le catalogue de features attachables (enregistrées au registre, activables/positionnables indépendamment), pas l'empilage analytique pour la complétude. Cf. mémoire `feedback_prioritize_modularity.md`. C'est pourquoi P4.10 est un **panel séparé** de P4.1 (composables indépendamment).
- **➡️ PROCHAIN BLOC (choisir SEUL)** : **P4.12 stats opérationnelles** (turnover, holding period, hit ratio, win/loss, yield-on-cost — moteur de lots P3.3a, dérivable sans snapshots ; NB se peuple avec des transactions réelles). Ou enrichir le **catalogue dashboard** (toutes les features récentes sont sur la fiche actif). ⚠️ **P4.2+ (TWR/MWR/vol/Sharpe portefeuille) bloqués sur l'accrual de snapshots SQLite** (table présente, non peuplée) — factualité : ne pas fabriquer de série de valeur portefeuille avant. Lire `ROADMAP_PM.md` § Prochain bloc.
- **P4.1+P4.10 en bref** : `src/utils/returnsCalculator.js` (cumulé/CAGR/matrice période/mensuels) + `returnsDistribution.js` (% positifs, best/worst, σ, skew/kurtosis, histogramme) + `returnsFormatters.js`, deux panels surface actif (`ReturnsMatrixPanel`, `ReturnsDistributionPanel`), registre catégorie `performance` (order 100/110), `layoutEngine` priorité 15. Tous dérivés de `/api/history` (days=1825) — **zéro nouveau fichier serveur**. Rendements de prix hors dividendes (mention affichée), périodes/mesures hors-portée **masquées**.
