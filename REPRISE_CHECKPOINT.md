...
- **🧪 CHANTIER SEED DÉMO (dev-only) — bloc « prixCourant » livré 2026-06-03, 986 tests verts.** Faux portefeuilles de démo pour exercer l'UI (`src/seed/profils.seed.js` déclaratif + `src/seed/seedRunner.js` idempotent + `DemoSeedPanel` dans Paramètres, tout sous `import.meta.env.DEV`). **Bloc livré ce jour** : champ `prixCourant` (map `SYMBOLE→prix` par profil) — corrige le bug visible où les titres `.TO` canadiens (non cotés par le free tier) affichaient **valeur 0 / faux P&L −100 %**. `derivePositions(transactions, method, prices)` sème un prix de référence statique au lieu de `0` ; une cotation live (titres US) l'écrase au montage, sinon le merge l'étiquette « données statiques conservées » (jamais présenté comme live → factualité OK). Prix ajoutés aux 2 profils CAD (Sophie : RY/TD/ENB/BCE.TO ; Julien : SHOP/AC.TO). +4 tests. **Reste du chantier seed (candidats next, par ordre logique)** : (2) générateurs A (50+ positions, RNG à graine fixe pour déterminisme) + B (historique long) ; (3) snapshots démo étiquetés « reconstitués » pour débloquer la **Performance** (~18 mois, contourne la limite free tier de l'historique) ; (4) resolver de prix historique quand `price` est omis dans une transaction (aujourd'hui tous les prix sont fournis). Hard rules : factualité, palette gelée, TDD, seed dev-only.
