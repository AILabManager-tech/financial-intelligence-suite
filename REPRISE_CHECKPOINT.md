...
**Recommandation par défaut sur « on continue »** : **P0.5 — profils de gestionnaire (presets)**, dernier bloc de Phase 0. Bundles nommés d'origine (« Vue d'ensemble », « Value investor », « Trader », « Conseiller client ») = set de features visibles + ordre par surface, applicable en 1 clic depuis `/settings` puis ajustable ; profils custom sauvegardables (localStorage `fis:profiles:v1`). Appliquer un profil = produire un layout complet et le commiter via les contrôles du contexte (P0.4). Presets en données pures (testables) + sélecteur dans `SettingsPage` + save/delete custom. Effort M, dépend de P0.1→P0.4 (livrés). Clôt Phase 0.
...
- **P0.5** profils de gestionnaire (presets « Vue d'ensemble / Value / Trader / Conseiller client », applicables en 1 clic, custom sauvegardables) — *recommandation par défaut, ci-dessus*. Dernier bloc Phase 0.
