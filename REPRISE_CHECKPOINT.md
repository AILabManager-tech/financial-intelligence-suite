...
- **`main` = `c8f396e`, P3.4 livré et poussé sur `origin/main`** (0 commit local d'écart). Branche `main`.
- **584 tests verts**, lint + build verts. Node v24 via nvm : `export PATH="$HOME/.nvm/versions/node/v24.14.1/bin:$PATH"`.
- **Avancement** : 🏁 Phase 0 (noyau personnalisable) · Phase 1 P1.1 (agencement déterministe ; **P1.2 IA gelé**) · 🏁 Phase 2 (simulateur de démo) · **Phase 3 en cours** : P3.1 migrations ✅, P3.2 multi-portefeuilles ✅ (client-first, vérifié live), P3.3a moteur de lots pur ✅, P3.3b stockage transactions + journal UI ✅ (route `/transactions`, vérifié live FIFO/LIFO), P3.2c parité dev SQLite multi-portefeuille ✅ (dev-only, vérifié curl), P3.4 multi-devises + FX ✅ (conversion vers devise base du mandat, vérifié curl).
- **➡️ PROCHAIN BLOC : clôture Phase 3** — (option A, recommandé) brancher `transactionStore` côté serveur (repo + API dev) pour la parité dev des transactions, dernier reste connu de Phase 3 ; ou (option B) attaquer la Phase 4 (analytics PM : returns/ratios/turnover via le moteur de lots P3.3a). Trancher A d'abord (ferme Phase 3 proprement).
- **Reste Phase 3** : parité dev des transactions (repo + API serveur autour de `003_transactions.sql`, table déjà créée). Puis Phase 3 close.
...
> **P3.4 livré (multi-devises + FX) — 2026-05-30 (niveau 3 MISSION).** `c8f396e`. Première vraie nouveauté fonctionnelle depuis P3.2 : conversion des totaux portefeuille (valeur/coût/P&L latent) de la devise de reporting USD vers la devise base du mandat, via taux ECB live. (1) `server/fx.js` pur : `fetchFxRates(base)` provider ECB keyless (Frankfurter) + fallback keyed (exchangerate.host) via `firstSuccessfulProvider`, map de taux ancrée sur la base (base = 1) ; `convertAmount` pur → `null` si une jambe manque (jamais de taux inventé). +8 tests. (2) `api/fx.js` (prod, mémo-cache 6h) + middleware dev `/api/fx?base=<ccy>` (`readThroughCache`, TTL 6h). (3) `src/utils/fxConvert.js` client pur (+6 tests). (4) `src/services/fx.js` client `AbortSignal` (+3 tests). (5) `CurrencyExposurePanel` (surface dashboard, enregistré, order 80) : convertit valeur/coût/P&L latent vers la devise base, affiche source + date du taux, masque (—) si taux manquant, ne rend rien si base = USD. Palette FIS gelée. +3 tests. (6) Sonde healthcheck `frankfurter.app`/`fx_rates` (+3 tests). Câblage `App.jsx` : `getActivePortfolio(portfolioList).baseCurrency` → panel. **Vérifié curl** (dev :20000) : `/api/health` provider fx `ok` ; `/api/fx?base=USD` → 30 taux ECB (CAD/USD 1.3805) ; `base=CAD` ancre CAD=1. Suite 561 → **584 tests verts**, lint+build verts, poussé. **Prochain bloc : clôture Phase 3 (parité serveur des transactions).**
