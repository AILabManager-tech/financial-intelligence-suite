...
- **🧪 CHANTIER SEED DÉMO (dev-only) — 2 blocs livrés 2026-06-03, 995 tests verts.** Faux portefeuilles de démo pour exercer l'UI (`src/seed/profils.seed.js` déclaratif + `src/seed/seedRunner.js` idempotent + `DemoSeedPanel` dans Paramètres, tout sous `import.meta.env.DEV`).
  ...
  - **Reste du chantier seed** : (2) générateurs A (50+ positions, RNG à graine fixe) + B (historique long) ; (4) resolver de prix historique quand `price` est omis dans une transaction (aujourd'hui tous les prix sont fournis). Hard rules : factualité, palette gelée, TDD, seed dev-only.
