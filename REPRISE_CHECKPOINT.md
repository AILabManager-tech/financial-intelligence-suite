...
- **`main` = P5.10 corrélation actif (ce bloc, NON poussé).** `origin/main` = `17efb32` (P5.9 drawdown + tout le bloc précédent poussés). ⚠️ **P5.10 = commit de ce bloc, NON poussé** (1 commit devant origin). Push après simple avertissement, jamais `--force`. Branche `main`.
- **714 tests verts** (P5.9 : →701 ; P5.10 : 701→714, +13 : 8 util + 5 panel), lint + build verts. Node v24 via nvm : `export PATH="$HOME/.nvm/versions/node/v24.14.1/bin:$PATH"`.
- **Avancement** : 🏁 Phase 0 (noyau personnalisable) · Phase 1 P1.1 (agencement déterministe ; **P1.2 IA gelé**) · 🏁 Phase 2 (simulateur de démo) · 🏁 Phase 3 (socle données PM) · **Phase 4 : features hors-snapshots livrées (P4.1 ✅ + P4.10 ✅ + P4.12 ✅, câblage P4.12 corrigé)** · **Phase 5 en cours : P5.1 journal ✅ + P5.8 concentration ✅ + P5.9 drawdown ✅ + P5.10 corrélation ✅**.
- **Cap autonome (directive user 2026-05-30)** : en l'absence de l'utilisateur, **prioriser la modularité/personnalisation** — enrichir le catalogue de features attachables (enregistrées au registre, activables/positionnables indépendamment), pas l'empilage analytique. Cf. mémoire `feedback_prioritize_modularity.md`. P5.10 est choisi car **purement additif et dérivé de `/api/history`** (même source que returns-matrix/distribution/drawdown), zéro source neuve.
- **➡️ PROCHAIN BLOC (choisir SEUL)** : (A) **accrual de snapshots SQLite** (brique socle, débloque P4.2 TWR / P4.3 MWR / P4.4 vol / P4.5 Sharpe — table `snapshots` présente, non peuplée ; factualité : ne jamais fabriquer la série de valeur) — réservé à quand l'utilisateur veut le factsheet portefeuille. (B) feature de catalogue alignée modularité : P5.2 contraintes/compliance (M, mais valide à l'ajout/import = touche le flux existant), P5.4 watchlists thématiques (S mais mute l'existant), ou autre panel dashboard/fiche dérivé des positions (le filon dérivé-de-`/api/history` dashboard est désormais bien couvert : concentration P5.8, corrélation P5.10). **Défaut si hésitation : (B).** Lire `ROADMAP_PM.md` § Prochain bloc.
- **✅ DETTE P4.12 CORRIGÉE (ce bloc)** : `OperationalStatsPanel` était dans `DASHBOARD_FEATURE_COMPONENTS` mais absent de `dashboardPanelProps` → recevait `{}` → état vide permanent. Cause : oubli de câblage (les `transactions` étaient pourtant dans le scope de App, ligne 198). Fix : construction des props extraite dans `src/core/dashboardPanelProps.js` (pur) + `OperationalStatsPanel: { transactions, method: "fifo" }` ajouté. **Garde anti-récurrence** : test `dashboardPanelProps.test.js` qui vérifie que **chaque** panel dashboard du registre a une entrée de props (un futur panel oublié casse le test).
