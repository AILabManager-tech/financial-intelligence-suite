...
- **`main` = P5.1 (`66a7b7e`) poussé + P5.8 (ce bloc, concentration & diversification, NON poussé).** Au début de ce bloc, P4.12+P5.1+fix `.env.example` ont été poussés (`origin/main` = `37c041a`). ⚠️ **P5.8 = commit de ce bloc, NON poussé** (1 commit devant origin). Push après simple avertissement, jamais `--force`. Branche `main`.
- **689 tests verts** (P5.1 : →677 ; P5.8 : 677→689, +12 : 7 util + 5 panel), lint + build verts. Node v24 via nvm : `export PATH="$HOME/.nvm/versions/node/v24.14.1/bin:$PATH"`.
...
- **⚠️ DETTE REPÉRÉE (non corrigée)** : `OperationalStatsPanel` (P4.12) est dans `DASHBOARD_FEATURE_COMPONENTS` mais **absent de `dashboardPanelProps`** (App.jsx ~604) → reçoit `{}`, donc `transactions=[]` → état vide en permanence sur le dashboard. Câblage P4.12 probablement incomplet (les transactions ne sont peut-être pas dans le scope dashboard de App). À investiguer/corriger dans un bloc dédié.
