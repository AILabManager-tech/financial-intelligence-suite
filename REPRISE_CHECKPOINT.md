...
## 🟢 ÉTAT ACTUEL — DÉMARRER ICI (mis à jour 2026-06-03)

- **🧪 CHANTIER SEED DÉMO (dev-only) — 3 blocs livrés 2026-06-03, 1007 tests verts.** Faux portefeuilles de démo pour exercer l'UI (`src/seed/profils.seed.js` déclaratif + `src/seed/seedRunner.js` idempotent + `DemoSeedPanel` dans Paramètres, tout sous `import.meta.env.DEV`).
  ...
  - **Reste du chantier seed** : (4) resolver de prix historique quand `price` est omis dans une transaction (aujourd'hui tous les prix sont fournis dans les profils — le resolver permettrait des profils déclaratifs sans prix, résolus via `/api/history`). Hard rules : factualité, palette gelée, TDD, seed dev-only.
