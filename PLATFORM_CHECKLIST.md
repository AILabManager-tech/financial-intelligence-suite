...
- [x] **F-13 Horodatage de la source de secours (B11)** — stooq renvoie `d2`/`t2` sans décalage ; `new Date("…T21:45:00")` est lu en heure LOCALE, donc le même envoi donnait un instant différent selon le navigateur. Aucun fuseau inventé (ce serait de la provenance fabriquée) : seule la date est retenue, à minuit UTC, étiquetée `asOfPrecision: "day"`. L'âge en heures n'est plus annoncé pour cette source ; finnhub garde `"instant"`. Normaliseur extrait en `server/stooqQuote.js` (pur, testé) — il était dupliqué entre `api/_handlers/quotes.js` et `vite.config.js`, donc corrigible d'un seul côté.
- [x] **F-14 Échec de stockage silencieux (B4)** — les stores retombaient sur leurs valeurs par défaut dans un `catch {}` muet. Le repli reste (mieux vaut démarrer qu'un écran blanc), le silence part : `src/services/storageDiagnostics.js` consigne la panne, trace en console pour le diagnostic à distance, bandeau `role="alert"` qui nomme ce qui n'a pas pu être relu et précise que rien n'a été effacé. Ni la trace ni le bandeau ne recopient la donnée corrompue. **Réserve** : le bandeau n'est pas couvert par un test — il n'existe aucun harnais pour `App.jsx` ; la logique dessous l'est.
- [x] **F-15 Couche de domaine de la recherche (B5)** — `search` était la seule feature sans `server/<feature>.js`, sa logique recopiée inline dans `api/_handlers/search.js` et le middleware `vite.config.js`. Extraite en `server/search.js` (fetcher injectable, 9 tests dont l'absence de fuite de jeton dans les résultats ET dans les messages d'erreur). Les deux serveurs l'appellent ; 28 lignes de duplication retirées de `vite.config.js`. Validation de longueur et de clé faite AVANT tout appel réseau.
- [x] **F-16 Sonde d'accord entre sources de prix (B6)** — `server/priceSourceAgreement.js` compare la clôture précédente finnhub à la dernière clôture complète twelvedata, tolérance 0,5 %, borne inclusive. Deux **clôtures** : confronter le cours courant à une clôture quotidienne signalerait une anomalie chaque après-midi de séance. `summarizeMarketDataHealth` accepte désormais `degraded` — une divergence n'est pas une panne, mais elle doit teinter la santé. Sans les deux valeurs : `unknown`, jamais un accord supposé.
- [x] **F-17 Hypothèse du calcul Buffett rendue explicite (B3)** — sous des hypothèses uniformes, `calcIntrinsicValue` est linéaire en flux (VI = K × FCF), donc « marge de sécurité > 25 % » **est** arithmétiquement « P/FCF < (1−0,25)×K ». Par défaut K = 21, soit P/FCF < 15,75. `impliedPriceToFcfThreshold` (pur, 4 tests dont une vérification croisée avec `calcIntrinsicValue`) ; affiché dans le panneau et suivant les curseurs ; `buildBuffettSummary` porte `assumptions` pour étiqueter le score du tableau. Ni renommage ni repersonnalisation : le calcul était juste, c'est l'implicite qui promettait trop.
