...
| 10 | **Jargon EN dans erreurs FR** | Tous les services : `throw "<feature> unavailable (502)"`. Sur tout `.TO`, l'utilisateur voit un mélange FR + code HTTP EN. | 9 panels : `— {state.error}` retiré au profit d'un message FR propre ; l'erreur reste en state pour le debug. 4 tests inversés (vérifient l'**absence** du code HTTP). | ✅ fait |
| 11 | **`MarketPulse.jsx` = code mort** | Composant non importé, non registré, shape mock sans guards. | Supprimé. | ✅ fait |

## VAGUE 2 — Snapshots en localStorage (gros morceau gratuit, meilleur ROI)

| # | Panne | Cause | Fix retenu | Statut |
|---|-------|-------|------------|--------|
| 1 | **9 panels Perf/Risque morts pour le vrai utilisateur** | Snapshots = SQLite dev-only. `savePortfolioSnapshot`/`fetchPortfolioSnapshots` sont **no-op en prod** ([portfolioSnapshots.js:12,27](src/services/portfolioSnapshots.js#L12), [devBackend.js:7](src/services/devBackend.js#L7)). localStorage ne stocke que les positions. Prouvé en prod : `/api/portfolio/snapshots` → 404. | `src/services/snapshotStore.js` : série journalière en **localStorage** (`fis:snapshots:v1`), namespacée par mandat, idempotente par jour, cappée 120. App : charge localStorage (le mirror SQLite dev supersède si présent) à l'init + activation mandat ; l'accrual écrit localStorage (durable) + mirror SQLite. Débloque TWR/MWR/risque/ratios/VaR/benchmark/beta. **Zéro backfill** : se remplit à partir d'aujourd'hui (≥2 jours requis, honnête). | ✅ fait (+8 tests) |
| 8 | **Fan-out history dépasse le quota twelvedata (pire en prod)** | `CorrelationMatrixPanel.jsx:87` lance jusqu'à 10 `/api/history` simultanés. Le handler prod `api/_handlers/history.js` n'a **pas** de mémo-cache (le dev en a un 6h → masque le bug). | Mémo-cache par instance (TTL 6h, clé `symbol\|interval\|outputsize`) ajouté à `history.js` — collapse les requêtes répétées d'une même série dans la durée de vie d'une instance chaude. | ✅ fait (+3 tests) |
